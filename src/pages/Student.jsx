...
const Student = () => {
  ...
  return (
    ...
    <div className='w-full px-[30px] py-[20px]'>
        <div className='flex justify-between items-center flex-wrap border-b border-[#E5E5E5] pb-[12px]'>
            <div className='text-[22px] font-[700] text-[#000000]'>
                Students List
            </div>
            <div>
                 <button onClick={()=>setModelPopup(true)} type="submit" className="w-full text-white bg-[#FEAF00] hover:bg-primary-700 focus:ring-4 uppercase focus:outline-none focus:ring-primary-300 font-medium rounded-[4px] text-[14px] px-5 py-[12px] text-center">ADD NEW STUDENT</button>
            </div>
        </div>

        <div className="relative overflow-auto h-calc">
            <div>
                <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    <thead className="text-xs text-gray-700">
                        <tr>
                            <td  className="px-6 py-[22px] text-[#ACACAC] text-[12px font-[600]">
                                Name
                            </td>
                            <th scope="col" className="px-6 py-[22px]  text-[#ACACAC] text-[12px font-[600]">
                                Email
                            </th>
                            <th scope="col" className="px-6 py-[22px]  text-[#ACACAC] text-[12px font-[600]">
                                Phone
                    ...
                    <tbody>
                        ...
                        studentList.map((data, index) => (
                            ...
                            <tr key={index} className="bg-white">
                                <td className='px-6 py-[34px] text-[14px] font-[400] text-[#000000]'>
                                   {data.name}
                                </td>
                                <td className="px-6 py-[34px] text-[14px] font-[400] text-[#000000]">
                                    {data.email}
                                ...
                                <td className="px-6 py-[34px] text-[14px] font-[400] text-[#000000]">
                                    <div className='flex items-center justify-center gap-[33px]'>
                                        <span className='cursor-pointer' onClick={()=>setEditModelPopup(true)}>
                                        ...
                                        <span className='cursor-pointer' onClick={()=>SetDeleteModal(true)}>
                                             { DeleteModal?  <Delete setModelPopup={SetDeleteModal} studentList={studentList} setStudentList={setStudentList}  email={data.email}/>:null}
                            ...
                            <tr>
                                <td colSpan={6} className='py-[5px]'></td>
                            </tr>
                            { editModelPopup?  <Modal setModelPopup={setEditModelPopup} studentList={studentList} setStudentList={setStudentList} edit={true}  CurrentStudent={data}/>:null}
                            </>
                        ))
                    }
                    </tbody>
        ...
        { modelPopup ?  <Modal setModelPopup={setModelPopup} studentList={studentList} setStudentList={setStudentList} edit={false} />:null}
